...
test("gruppuppgiftens facit blir en EGEN fil, inte en sista sida",
  async ({ page }) => {
    /* Facit låg förut sist i gruppens ark, och det var inget val: mallen
       skrev alltid ut «Facit och bedömning». Läraren 2026-08-30: arket
       ligger på ett bord under lektionen, det är elevernas skrivyta, och
       lösningarna hör inte hemma på det. Gruppuppgiften fick därför samma
       facitväljare som arbetsbladet, med «Separat facit» som förval — och
       då hör bilderna hemma i `facit`, filen /api/exams/{id}/facit hämtar.

       «Facit i bladet» är kvar som val, och då står facitarket i dokumentet
       självt (blad.js `bladen`) och alltså i `uppgift` — precis där den
       gamla regeln lade det. */
    ...
    const iArket = await page.evaluate(() =>
      document.querySelectorAll('#arkskal .ark[data-form="fa"]').length);
    expect(iArket, "gruppens ark bär inte lärarens sida med förvalet")
      .toBe(0);
    ...
    expect(blad.facit.every(arPng)).toBe(true);
    expect(blad.uppgift.every(arPng)).toBe(true);
    // Två papper, inte ett: facitsidan är inte en av gruppens sidor.
    expect(blad.facit[0]).not.toBe(blad.uppgift[0]);
  });
