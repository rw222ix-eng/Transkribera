...
const __ds_ns = (window.TranskriberaDesignSystem_c5f72b = window.TranskriberaDesignSystem_c5f72b || {});

const __ds_scope = {};

(__ds_ns.__errors = __ds_ns.__errors || []);

// bedomning/card-modal.js
try { (() => {
/* ============================================================
...
(function cardZoomModal() {
  ...
  style.textContent = ['.cardzoom{position:fixed;inset:0;z-index:1000;cursor:zoom-out;', 'background:rgba(20,22,18,0);', '-webkit-backdrop-filter:blur(0px);backdrop-filter:blur(0px);', 'transition:background .5s ' + EASE + ',backdrop-filter .5s ' + EASE + ';}', '.cardzoom.is-on{background:rgba(20,22,18,0.55);', '-webkit-backdrop-filter:blur(7px) saturate(.92);backdrop-filter:blur(7px) saturate(.92);}', '.cardzoom_card{transform-origin:top left;', 'box-shadow:0 44px 90px -24px rgba(0,0,0,.5),0 18px 44px -20px rgba(0,0,0,.38);}', '.cardzoom-open{cursor:zoom-in;}',
  ...
  '.cz-frozen .scard,.cz-frozen .flow_step,.cz-frozen .scard_block{', 'transition:none!important;}', '.cz-frozen .scard,.cz-frozen .flow_step{', 'filter:none!important;opacity:1!important;transform:none!important;}', '.cz-frozen .scard_block{transform:none!important;box-shadow:none!important;', 'border-color:var(--line)!important;}'].join('');
  ...
  function rowOf(card) {
    return card.closest('.flow, .features-row') || card.parentElement;
  }
  ...
  function syncAnimations(srcRoot, cloneRoot) {
    ...
    var src, cln;
    try {
      src = srcRoot.getAnimations({
        subtree: true
      });
      cln = cloneRoot.getAnimations({
        subtree: true
      });
    } catch (e) {
      return;
  ...
  function unfreezeOnMove(row) {
    ...
    document.addEventListener('wheel', release, {
      passive: true,
      once: true
    });
    document.addEventListener('touchstart', release, {
      passive: true,
      once: true
    });
  }
  function open(card) {
    ...
    var scaledW = rect.width * f,
      scaledH = rect.height * f;
    var finalLeft = (vw - scaledW) / 2,
      finalTop = (vh - scaledH) / 2;
    var tx = finalLeft - rect.left,
      ty = finalTop - rect.top;
  ...
  function close() {
    ...
    var overlay = active.overlay,
      clone = active.clone,
      row = active.row,
      finalTransform = active.finalTransform;
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "bedomning/card-modal.js", error: String((e && e.message) || e) }); }
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "bedomning/site.js", error: String((e && e.message) || e) }); }
