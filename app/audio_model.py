"""The internal audio-grounded correction model (Gemma 4 E4B).

Gemma 4 E4B accepts native AUDIO input and runs via transformers on the GPU. It
...
`google/gemma-4-E4B-it` is OPEN (non-gated) — it downloads without a HuggingFace
token. This is the model we settled on as the best audio-correction engine.

⚠️ UNVERIFIED at the inference layer: the transformers audio inference in
audio_correct_cli has not yet been smoke-tested end-to-end on the RTX box. The
download + install wiring is verified.
...
AUDIO_MODEL_ID = "google/gemma-4-E4B-it"   # OPEN (non-gated); native audio; transformers + GPU
...
def download_audio_model(models_root: Path,
    ...
    if not is_audio_model_installed(models_root):
        raise RuntimeError("Nedladdning ofullständig: ljudmodellen (vikter/config saknas). "
                           "Kontrollera nätverksanslutning och diskutrymme och försök igen.")
    if progress_cb is not None:
        progress_cb(100)
    return target
