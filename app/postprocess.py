...
ANSWER_SYSTEM = (
    ...
    "1. SVARET FÖRST. Din första mening besvarar frågan rakt av. Finns det "
    "läraren frågar efter INTE i utdragen säger du det direkt och kort — "
    "t.ex. ”Nej — täljare och nämnare tas inte upp i någon av "
    "inspelningarna; den enda mattelektionen handlar om formler för dubbla "
    "vinkeln [1].” — och stannar där. Referera ALDRIG hela inspelningen "
    "när svaret är att något saknas: högst EN mening om vad den handlar om "
    "i stället.\n"
    "2. CITERA ORDAGRANT när svaret finns: varje viktigt påstående bärs av "
    "ett kort ordagrant citat ur utdraget inom citattecken, och namnge vem "
    "som säger vad när det framgår. Sammanhang (lektion, sketch, klipp, "
    "möte — avgör ur namn och innehåll, och anta aldrig att talaren är "
    "läraren; komik läses inte som allvar) vävs in där det behövs för att "
    "svaret inte ska missförstås — inte som en obligatorisk inledning.\n"
    ...
    "4. BARA UTDRAGEN. Hitta inte på; är du osäker säger du det.\n"
    ...
    "Exempel på TON vid träff: ”Ja — i genomgången från 6 juli förklarar du "
    "det med ’sinus för dubbla vinkeln är två sinus x cosinus x’ [1] …”. "
    "Exempel vid miss: ”Nej — det tas inte upp; inspelningen är en sketch "
    "där NN driver med X [1].”"
)
...
def build_answer_prompt(query: str, excerpts: list[dict]) -> str:
    ...
    return (f"Fråga: {query}\n\n"
            ...
            f"{context}\n---\n\n"
            f"Besvara frågan direkt i första meningen. Saknas det frågan "
            f"gäller i utdragen: säg det rakt ut och beskriv med högst en "
            f"mening vad inspelningarna i stället handlar om — referera "
...
def edit_calendar_suggestion(query: str, context: str, cal_event: dict,
                             model: str,
