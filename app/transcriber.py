...
def build_parakeet_cmd(audio: Path, model_dir: str, language: str,
                       out_base: Path, formats: list[str]) -> list[str]:
    """Build the argv to run one Parakeet (onnx-asr) transcription in an isolated
    subprocess, mirroring build_transcribe_cmd's stdout protocol.

    Parakeet always runs on the GPU (CUDA execution provider) — there are no
    device/compute_type knobs, unlike the faster-whisper path. Frozen builds
    re-invoke the exe with the `parakeet-cli` subcommand.
    """
    if getattr(sys, "frozen", False):
        head = [sys.executable, "parakeet-cli"]
    else:
        head = [sys.executable, "-m", "app.parakeet_cli"]
    return head + [
        "--audio", str(audio),
        "--model-dir", model_dir,
        "--language", language or "",
...
def build_audio_correct_cmd(audio: Path, model_dir: str, segments_json: str,
    ...
    (Gemma 4 E4B via transformers on the GPU). Same stdout protocol as the
    transcription CLIs; segments are passed as a JSON file path.
    """
    if getattr(sys, "frozen", False):
